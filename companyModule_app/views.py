...
from common.serializers import CompanyModuleSerializer
from common.swagger_utils import get_api_response_serializer
from .service import CompanyModuleService

logger = logging.getLogger(__name__)
module_service = CompanyModuleService()

@extend_schema(
    summary="Create Module",
    description="Add a new company module and assign policies.",
    request=CompanyModuleSerializer,
    responses={
...
def create_module(request):
    try:
        result = module_service.create_module(request.data)
...
def all_module_list_page(request):
    try:
        ...
        result = module_service.all_module_list_page(search_key, page, size)
...
def module_by_functionality_list_page(request):
    try:
        ...
        result = module_service.module_by_functionality_list_page(functionality_id, search_key, page, size)
        return ApiResponse(status.HTTP_200_OK, "Modules list fetched successfully", result)
    except Exception as e:
...
@extend_schema(
    ...
    responses={
        200: get_api_response_serializer(CompanyModuleSerializer, name="ModuleResponse"),
        500: get_api_response_serializer(name="ErrorResponse")
    }
)
@api_view(['GET'])
...
def get_module_by_id(request, moduleId):
    try:
        ...
        result = module_service.get_module_by_id(module_id)
...
@extend_schema(
    ...
    request=CompanyModuleSerializer,
    responses={
        200: get_api_response_serializer(CompanyModuleSerializer, name="ModuleUpdatedResponse"),
        500: get_api_response_serializer(name="ErrorResponse")
    }
)
@api_view(['PUT'])
...
def update_module_by_id(request, moduleId):
    try:
        module_id = int(moduleId)
        result = module_service.update_module_by_id(module_id, request.data)
...
def delete_module_by_id(request, moduleId):
    try:
        module_id = int(moduleId)
        module_service.delete_module_by_id(module_id)
