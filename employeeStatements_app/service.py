...
class EmployeeSalaryStatementService:
    ...
    def build_employee_salary_statement(self, company_employee: CompanyEmployee, salary_statement_request_dto: dict) -> dict:
        ...
        logger.info(f"Start Date: {start_date_utc} (Local Date: {start_local_date})")
        logger.info(f"End Date: {end_date_utc} (Local Date: {end_local_date})")
        logger.info(f"Paid Days: {total_paid_days_count}")
        logger.info(f"Worked Days: {len(actual_work_days)}")
        logger.info(f"Total Worked Days: {len(actual_work_days) + total_paid_days_count}")
        logger.info(f"Total Worked Minutes: {total_worked_minutes}")
        logger.info(f"Overtime Minutes: {ot_final_minutes}")
        logger.info(f"Overtime Amount: {ot_amount_final}")
        logger.info(f"Total Earnings: {total_earnings}")
        logger.info(f"PF Amount: {pf_amount}")
        logger.info(f"PT Amount: {pt_amount}")
        logger.info(f"Penalty Amount: {penalty_amount}")
        logger.info(f"Allowance: {total_allowance}")
        logger.info(f"Deductions: {total_deduction_amount}")
        logger.info(f"Other Deductions (Canteen + Penalty): {other_deductions}")
        logger.info(f"Total Deductions: {total_deductions}")
        logger.info(f"Net Salary: {total_earnings - total_deductions}")
